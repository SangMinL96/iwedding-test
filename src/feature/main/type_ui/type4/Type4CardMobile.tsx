...
const Type4CardMobile = ({ item, imgBelowExpose, thumbnailRatio, orderBadgeFlag, indexNum, isLike = false }: PropsType) => {
  ...
  return (
    <Container href={item.url.includes('https://') ? item.url : `https://www.iwedding.co.kr${item.url}`} target={item.target}>
      <div className={item.type === 'brand' ? 'img_box small' : 'img_box'}>
        <Image unoptimized layout='fill' alt='productImg' src={urlReplace(item.img_mobile)} />
      ...
      <div className={item.type === 'brand' ? 'text_box_2021 prd_info_box vertical_center' : 'text_box_2021 prd_info_box'}>
        ...
        {/* title 구분 */}
        {item.type === 'product' && <p className='prd_title product'>{item.title}</p>}
        {item.type === 'content' && <p className='prd_title content'>{item.title}</p>}
        {item.type === 'brand' && <p className='prd_title brand'>{item.title}</p>}
        {item.type === 'event' && <p className='prd_title'>{item.title}</p>}

        {/* sub_title 구분 */}
        {item.sub_title !== '' && (
          <span className={item.type === 'content' ? 'prd_sub_title content' : 'prd_sub_title'}>{item.sub_title}</span>
        )}
        {/* description_box, hashtag_box 넣어야할듯? */}
...
export default Type4CardMobile;
