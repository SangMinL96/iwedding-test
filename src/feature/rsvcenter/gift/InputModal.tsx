...
const isValidateData = {
  easy_book_who: true, //스크롤 유효성 검사시 해당 컬럼값은 예복에 존재하지않아 미리 트루로 변경
  easy_book_which_product: false,
  'easy_book.type1_budget': false,
  'easy_book.type2_budget': true, //스크롤 유효성 검사시 해당 컬럼값은 예복에 존재하지않아 미리 트루로 변경
  'easy_book.when_check': false,
  'easy_book.which_style': false,
  'easy_book.cnt': false,
  'easy_book.fitting': true, //스크롤 유효성 검사시 해당 컬럼값은 예복에 존재하지않아 미리 트루로 변경
...
function InputModal({ isComplete, isDone }: PropsType) {
  ...
  useEffect(() => {
    if (!from) {
      if (
        isValidate['easy_book_which_product'] &&
        isValidate['easy_book.type1_budget'] &&
        isValidate['easy_book.when_check'] &&
        isValidate['easy_book.which_style'] &&
