...
function Content4({ setIsValidate }) {
  ...
  useEffect(() => {
    ...
    setValue('easy_book.cnt', values.list1);
    setValue('easy_book.fitting', values.list2);
  ...
  useEffect(() => {
    if (data?.every(item => !item)) {
      ...
      setValues(prev => ({ ...prev, list2: null }));
    }
  }, [data]);
  ...
  return (
    <ContentBox>
      <h3 id='which_style'>어떤 스타일을 선호하시나요?</h3>
      <p>선호하는 스타일에 맞춰 예복샵을 추천해드릴게요.</p>

      <h5 id='cnt'>Q. 업체 몇 군데를 비교하고 싶으신가요?</h5>

      {robesList.content4_2.map(item => (
      ...
      {data?.some(item => item) && (
        <>
          <h5 id='fitting'>Q. 대여예복 상담 시, 직접 피팅을 원하시나요?</h5>
          {robesList.content4_3.map((item, index) => (
            <InputBox key={item.value}>
              <SelectButton
                id={item.value}
      ...
      <h5 id='easy_book_which_ent_type'>
        Q. 어떤 예복샵을 방문하고 싶으신가요? <span>(최대 3개)</span>
      </h5>
      {robesList.content4_4.map(item => (
        <InputBox key={item.value}>
          <SelectButton
            id={item.value}
