      ...
      {values.find(item => item.cnt > 0) && (
        <>
          ...
          <SubContentBox>
            ...
            <DescBox>
              <p>맞춤 한복: 체형과 취향에 따라 맞춤 제작하는 방식</p>
              <p>대여 한복: 미리 제작된 한복을 일정 기간 대여하는 방식</p>
            </DescBox>
            {values.map(item => {
              if (item.cnt > 0) {
                // 신랑1,신랑2 와 같은 배열안에 새로운 객체들이 따로 들어가서 대여 or 맞춤 선택해줘야하기때문에 새로운 배열을 만들고 그거를 map로 다시돌림
                const cntArray = Array.from({ length: item.cnt }, (v, i) => i);
                return cntArray.map(cnt => (
                  <InputBox key={item.value}>
                    <SelectButton style={{ width: '60%', border: 'none', backgroundColor: '#F5F5F5' }}>
                      {item.name}&nbsp;{cnt + 1}
                    </SelectButton>
